...
class DateUtils {
  ...
  ///获取当前时间戳
  getTdayTimeMillisecondsSinceEpoch() {
    var today = DateTime.now();
  ...
  ///calendar 时间差
  ///格式 2020/10/05
  fromMillisecondsSinceEpoch(date, {int calendar = 0}) {
    var a = date.millisecondsSinceEpoch; //时间戳
    return DateTime.fromMillisecondsSinceEpoch(a + calendar).year.toString() +
        '/' +
        DateTime.fromMillisecondsSinceEpoch(a + calendar).month.toString() +
        '/' +
        DateTime.fromMillisecondsSinceEpoch(a + calendar).day.toString();
  }
  fromEpoch(date, {int calendar = 0}) {
    var a = date.millisecondsSinceEpoch; //时间戳
    return DateTime.fromMillisecondsSinceEpoch(a + calendar).year.toString() +
        '-' +
        DateTime.fromMillisecondsSinceEpoch(a + calendar).month.toString() +
  ...
  ///获取 时间差 传入格式 2019-10-20 10:10:10
  String getTimeBettwenStartTimeAndEnd({startTime: String}) {
    if (startTime == null || startTime == '') {
      return '';
    }
    var today = DateTime.now();
    var diffTime = today.difference(DateTime.parse(startTime));
    List time = diffTime.toString().split(':');
    print(diffTime.toString());
    if (time.length == 3) {
      if (int.parse(time[0]) < 24) {
        return (time[0] + '小时' + time[1] + '分').toString();
      } else {
        return ((int.parse(time[0]) / 24).toString()).substring(
                0, ((int.parse(time[0]) / 24).toString()).indexOf('.')) +
            '天' +
            (int.parse(time[0]) % 24).toString() +
            '小时' +
            time[1] +
            '分';
      }
    } else {
      return time[0].toString() + '分';
    }
  }

  ///获取 小时差 传入格式 2019-10-20 10:10:10
  int getTimeDay({startTime: String}) {
    if (startTime == null || startTime == '') {
      return 0;
    }
    var today = DateTime.now();
  ...
  ///获取 时间差 传入格式 2019-10-20 10:10:10
  int getBettwenStartTimeAndEnd(String startTime, String endTime) {
    var riskDate = DateTime.parse(endTime);
    var damageTime = DateTime.parse(startTime);
    List time = damageTime.difference(riskDate).toString().split(':');
    if (time.length == 3) {
      return int.parse(time[0]);
    } else {
      return 0;
